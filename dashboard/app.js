...
function renderPerformanceMetrics(data) {
  ...
  $("#node-metrics-table").innerHTML = nodes.filter((item) => !selected || item.task_id === selected).sort((a, b) => (b.duration_seconds || 0) - (a.duration_seconds || 0)).map((item) => `<div class="metrics-row"><strong>${esc(item.label || shortId(item.task_id))}</strong><span>${esc(item.type || "node")}</span><span>${esc((item.tool_methods || []).join(" · ") || "sin herramienta")}</span><span>${formatSeconds(item.duration_seconds)}</span><span>${formatNumber(item.actual_tokens_available ? item.actual_tokens : item.estimated_tokens)} tokens</span><span>${item.llm_calls || 0} LLM · ${item.tool_calls || 0} tools · ${item.retry_count || 0} reintentos</span></div>`).join("") || '<div class="empty">Sin métricas de nodos.</div>';
...
function workflowCard(kind, title, detail, status = "READY", meta = "", attrs = "") {
  const glyphs = { start: "→", codegraph: "⌘", orchestrator: "◎", worker: "◌", task: "●", operation: "•", subtask: "└", delegate: "↳", finish: "✓", next: "→" };
  const text = String(title || "");
  const displayTitle = text.length > 120 ? `${text.slice(0, 117)}...` : text;
  return `<div class="workflow-node workflow-${esc(kind)} status-${esc(status)}" ${attrs}><span class="workflow-kind" title="${esc(kind)}">${glyphs[kind] || "•"}</span><div><strong title="${esc(text)}">${esc(displayTitle)}</strong><small>${esc(detail)}</small>${meta ? `<em>${esc(meta)}</em>` : ""}</div></div>`;
}
